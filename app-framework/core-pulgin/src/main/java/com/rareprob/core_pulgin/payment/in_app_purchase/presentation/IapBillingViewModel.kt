...
@HiltViewModel
class IapBillingViewModel @Inject constructor(application: Application, coreDatabase: CoreDatabase) :
    ...
    fun startBillingConnection(rcProductItemList: List<ProductListingData> = emptyList()) {
    ...
    fun getInAppPacksFromRc() {
        viewModelScope.launch {
            repo.getInAppProducts().onEach { result ->
                when (result) {
                    is Resource.Success -> {
                        _rcProductItemList.value = result.data ?: emptyList()
